...
$data_srtmsk = mysqli_query($conn, "SELECT * FROM surat_masuk WHERE id_surat = $id")->fetch_assoc();
$footer = mysqli_query($conn, "SELECT * FROM komponen WHERE nama_komponen = 'footer'")->fetch_assoc();

if(isset($_POST["ubah"])) {
    if(ubah_srtmsk($_POST, $id) > 0){
        echo "<script>
...
<body>
    ...
    <nav class="navbar navbar-expand-lg navbar-light border-bottom">
        <div class="container-xl">
            ...
            aria-controls="main-nav" aria-expanded="false" aria-label="Toggle Navigation">x 
            ...
            <div class="collapse navbar-collapse justify-content-end align-items-center" id="main-nav">
                <ul class="navbar-nav">
                    <li class="navbar-item">
                        <a href="surat_masuk.php" class="nav-link" style="font-weight: 600;">Surat Masuk</a>
                    </li>
                    <li class="navbar-item">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="" method="post" enctype="multipart/form-data">
                            <input type="hidden" name="fileLama" value="<?= $data_srtmsk["file"]; ?>">
                            <div>
                                <label for="no_surat" class="form-label">Nomor Surat</label>
                                <input type="text" name="no_surat" id="no_surat" class="form-control" required autocomplete='off' autofocus value="<?= $data_srtmsk["no_surat"]; ?>">
                            </div>
                            <div class="mt-3">
                                <label for="tgl_surat" class="form-label">Tanggal Surat</label>
                                <input type="date" name="tgl_surat" id="tgl_surat" class="form-control" required autocomplete='off' value="<?= $data_srtmsk["tgl_surat"]; ?>">
                            </div>
                            <div class="mt-3">
                                <label for="perihal" class="form-label">Perihal</label>
                                <input type="text" name="perihal" id="perihal" class="form-control" required autocomplete='off' value="<?= $data_srtmsk["perihal"]; ?>">
                            </div>
                            <div class="mt-3">
                                <label for="kepada" class="form-label">Kepada</label>
                                <input type="text" name="kepada" id="kepada" class="form-control" required autocomplete='off' value="<?= $data_srtmsk["kepada"]; ?>">
                            </div>
                            <div class="mt-3">
                                <label for="file" class="form-label">File PDF</label>
                                <input type="file" name="file" id="file" class="form-control" value="<?= $data_srtmsk["file"]; ?>">
                                <a href=""><?= $data_srtmsk["file"]; ?></a>
                            </div>
